...
function createCrownTexture(themeColorHex) {
  ...
  // 1. Royal Crown Body with curved cartoon silhouette (Color-matched to avatar)
  ctx.beginPath();
  ctx.moveTo(-62, 32);
  // Left wing tip
  ...
  // Royal rich gradient dynamically tailored to avatar's theme color
  const baseColor = new THREE.Color(themeColorHex);
  const crownGrad = ctx.createLinearGradient(0, -50, 0, 36);
  crownGrad.addColorStop(0.0, '#FFFFFF');
  crownGrad.addColorStop(0.25, baseColor.clone().lerp(new THREE.Color(0xffffff), 0.55).getStyle());
  crownGrad.addColorStop(0.60, themeColorHex);
  crownGrad.addColorStop(1.0, baseColor.clone().multiplyScalar(0.42).getStyle());
  ...
  // 3. Radiant Golden & Avatar Energy Star on Center Peak
  function drawStar(cx, cy, spikes, outerRadius, innerRadius) {
    ...
    starGrad.addColorStop(0.40, baseColor.clone().lerp(new THREE.Color(0xffffff), 0.65).getStyle());
    starGrad.addColorStop(1.0, themeColorHex);
...
/**
 * Creates authentic Doctor Strange / Cosmic Portal Ring texture with fiery sparks
 */
function createPortalRingTexture(themeColorHex) {
  if (typeof document === 'undefined') return null;
  const canvas = document.createElement('canvas');
  canvas.width = 512;
  canvas.height = 512;
  ...
  // Outer cosmic fiery glow ring
  const grad = ctx.createRadialGradient(cx, cy, 90, cx, cy, 250);
  grad.addColorStop(0.0, 'rgba(0,0,0,0)');
  grad.addColorStop(0.35, 'rgba(255, 140, 0, 0.35)');
  grad.addColorStop(0.60, '#F59E0B'); // Golden amber
  grad.addColorStop(0.76, '#FDE047'); // Brilliant gold
  grad.addColorStop(0.85, '#FFFFFF'); // White hot spark core
  grad.addColorStop(0.92, (themeColorHex || '#8b5cf6') + 'aa');
  grad.addColorStop(1.0, 'rgba(0,0,0,0)');
  ctx.fillStyle = grad;
  ctx.fillRect(0, 0, 512, 512);

  // Dense fiery sparks and spray embers orbiting the ring like Doctor Strange portal
  const sparkColors = ['#FFFFFF', '#FFF4CC', '#FDE047', '#F59E0B', '#EA580C', '#D97706'];
  for (let i = 0; i < 90; i++) {
    const angle = (i / 90) * Math.PI * 2 + (Math.random() - 0.5) * 0.25;
    const r = 200 + (Math.random() - 0.5) * 36;
    const sx = cx + Math.cos(angle) * r;
    const sy = cy + Math.sin(angle) * r;
    ctx.fillStyle = sparkColors[Math.floor(Math.random() * sparkColors.length)];
    ctx.beginPath();
    ctx.arc(sx, sy, 2.0 + Math.random() * 4.5, 0, Math.PI * 2);
    ctx.fill();
    ...
    ctx.beginPath();
    ctx.moveTo(sx, sy);
    ctx.lineTo(sx - Math.sin(angle) * (8 + Math.random() * 16), sy + Math.cos(angle) * (8 + Math.random() * 16));
    ctx.stroke();
  }
...
/**
 * Creates open cosmic space cluster aperture inside the portal ring
 */
function createCosmicVoidTexture(themeColorHex) {
  if (typeof document === 'undefined') return null;
  const canvas = document.createElement('canvas');
  canvas.width = 512;
  canvas.height = 512;
  ...
  // Pure pitch black void inside circular aperture
  ctx.fillStyle = '#000000';
  ctx.beginPath();
  ctx.arc(cx, cy, 210, 0, Math.PI * 2);
  ctx.fill();

  // Star clusters & cosmic stardust inside void aperture
  const starColors = ['#ffffff', '#bae6fd', '#ddd6fe', '#fef08a', '#a5f3fc'];
  for (let i = 0; i < 50; i++) {
    const angle = Math.random() * Math.PI * 2;
    const r = Math.random() * 185;
    const sx = cx + Math.cos(angle) * r;
    const sy = cy + Math.sin(angle) * r;
    ctx.fillStyle = starColors[Math.floor(Math.random() * starColors.length)];
    ctx.beginPath();
    ctx.arc(sx, sy, 0.8 + Math.random() * 2.2, 0, Math.PI * 2);
    ctx.fill();
  }
...
/**
 * Creates subtle 3D wavy particle system inside chamber portal aperture
 */
function createPortalWaveMesh(themeColorHex) {
  const geo = new THREE.PlaneGeometry(2.1, 2.1, 20, 20);
  const mat = new THREE.PointsMaterial({
    color: new THREE.Color(themeColorHex || '#8b5cf6'),
    size: 0.045,
    transparent: true,
    opacity: 0.65,
  ...
  const orig = new Float32Array(count * 2);
  const phases = new Float32Array(count);
  for (let i = 0; i < count; i++) {
    orig[i * 2] = geo.attributes.position.getX(i);
    orig[i * 2 + 1] = geo.attributes.position.getY(i);
    phases[i] = Math.random() * Math.PI * 2;
  }
  mesh.userData = { orig, phases };

  return mesh;
}

/**
 * AvatarManager — High-Performance Multi-Avatar Chamber Engine
 * Matches the reference design with:
 *  - 4 AI Companions on glowing circular pedestals:
 *      0: Mowgli / Ask Vedika (Purple)
 *      1: Belle / Coding Tutor (Red)
 *      2: Moana / Viva Interview (Gold)
 *      3: Bhageera / Dynamic Presentation (Blue)
 *  - Organic 2nd-order spring physics
 *  - Natural breathing, eye tracking, and head tilting
...
export class AvatarManager {
  constructor(scene, camera, chambers, onReady = null) {
    ...
    if (!cachedAvatarTextures) {
      ...
      cachedAvatarTextures = [
        textureLoader.load('/avatar_purple.webp?v=5'),
        textureLoader.load('/avatar_red.webp?v=5'),
        textureLoader.load('/avatar_gold.webp?v=5'),
        textureLoader.load('/avatar_blue.webp?v=5'),
      ];
    ...
    // Load color-matched cartoon crowns matching each avatar's specific color:
    if (!cachedCrownTextures) {
      const textureLoader = new THREE.TextureLoader();
      cachedCrownTextures = [
        textureLoader.load('/crown_purple.png?v=3'),
        textureLoader.load('/crown_red.png?v=3'),
        textureLoader.load('/crown_gold.png?v=3'),
        textureLoader.load('/crown_blue.png?v=3'),
      ];
      cachedCrownTextures.forEach((tex) => {
        tex.flipY = true; // Upright orientation for 3D quad
        tex.colorSpace = THREE.SRGBColorSpace;
      });
    }
    this.crownTextures = cachedCrownTextures;
  ...
  _instantiateAvatars() {
    this.chambers.forEach((chamber, idx) => {
      ...
      const baseScale = 2.65; // Slightly bigger avatars as requested
      if (maxDim > 0) {
      ...
      */
      ...
      // 6. Authentic Top Doctor Strange Portal Ring & Open Cosmic Void Aperture directly on top of avatar placement
      const portalGroup = new THREE.Group();
      const portalGeo = new THREE.PlaneGeometry(3.2, 3.2);
      const portalRingMat = new THREE.MeshBasicMaterial({
        map: createPortalRingTexture(data.themeColor),
        transparent: true,
        blending: THREE.AdditiveBlending,
        depthWrite: false,
        side: THREE.DoubleSide,
      });
      const portalRingMesh = new THREE.Mesh(portalGeo, portalRingMat);

      // Open cosmic space cluster inside aperture (user's point of view)
      const voidGeo = new THREE.CircleGeometry(1.04, 32);
      const voidMat = new THREE.MeshBasicMaterial({
        map: createCosmicVoidTexture(data.themeColor),
        depthWrite: false,
        side: THREE.DoubleSide,
      });
      ...
      // Subtle 3D wavy particle system inside chamber portal aperture
      const portalWaveMesh = createPortalWaveMesh(data.themeColor);
      if (portalWaveMesh) {
        portalGroup.add(portalWaveMesh);
      }

      // Positioned directly on top of the avatar placement (y = 2.55)
      portalGroup.position.set(0, 2.55, 0.15);
      portalGroup.scale.set(0.001, 0.001, 0.001); // starts closed
      chamber.group.add(portalGroup);

      const avatarLocalY = 0.0;
      avatarGroup.position.set(0, 2.55, 0.0); // start at portal height
      avatarGroup.scale.set(0.001, 0.001, 0.001); // start hidden
    ...
    if (this.pendingArrival) {
      ...
      this.pendingArrival = null;
      this.playPortalArrival(cb);
    }
  }

  playPortalArrival(onAllLanded) {
    if (!this.isReady || !this.avatars || this.avatars.length === 0) {
      this.pendingArrival = onAllLanded || true;
      return;
    }

    // Deep cosmic dimensional warp whoosh
    playDeepCosmicWhoosh(2.4, 1.0);

    const fallHeight = 2.55;

    // All avatars start invisible in their top portals in the back row
    this.avatars.forEach((av) => {
      av.avatarGroup.position.y = fallHeight;
      ...
      if (av.topPortalMesh) {
        av.topPortalMesh.scale.set(0.001, 0.001, 0.001);
        av.topPortalMesh.visible = true;
      }
    });

    const numAvatars = this.avatars.length;
    const dropDuration = 0.58;
    const staggerInterval = 0.52; // Distinct sequential entrance one after the other

    this.avatars.forEach((av, idx) => {
      const delay = 0.20 + idx * staggerInterval;
      const tl = gsap.timeline({ delay });

      // 1. Individual Doctor Strange portal swirls open with fiery sparks directly above this avatar's station
      tl.call(() => {
        playAvatarWhoosh(0.92 + idx * 0.12);
        if (av.topPortalMesh) {
          gsap.fromTo(
            av.topPortalMesh.scale,
            { x: 0.001, y: 0.001, z: 0.001 },
            { x: 1.0, y: 1.0, z: 1.0, duration: 0.32, ease: 'back.out(1.5)' }
          );
        }
      });

      // 2. Avatar drops out of portal with parabolic gravitational acceleration & stretch
      tl.call(() => {
        av.avatarGroup.visible = true;
      }, null, 0.15);

      tl.fromTo(av.avatarGroup.position, {
        y: fallHeight,
      }, {
        y: 0.0,
      ...
      tl.fromTo(av.avatarGroup.scale, {
        x: 0.2,
        y: 0.5,
        z: 0.2,
      }, {
      ...
      // 3. Portal swirls shut as avatar clears event horizon
      tl.call(() => {
        if (av.topPortalMesh) {
          gsap.to(av.topPortalMesh.scale, { x: 0.001, y: 0.001, z: 0.001, duration: 0.30, ease: 'power2.in' });
        }
      }, null, 0.40);

      // 4. Impact squash on striking pedestal disk in the back row!
      const landTime = 0.15 + dropDuration;
      ...
      // 6. Settle down onto back pedestal disk
      tl.to(av.avatarGroup.position, {
        y: 0.0,
        duration: 0.42,
        ease: 'bounce.out',
    ...
    // 7. ONLY ONCE ALL 4 AVATARS HAVE COME OUT OF THE PORTAL AND SETTLED IN THE BACK ROW:
    const totalLandingTime = 0.20 + (numAvatars - 1) * staggerInterval + 0.15 + dropDuration + 0.78;
    gsap.delayedCall(totalLandingTime, () => {
  ...
  playPortalExit(onComplete) {
    ...
    const staggerInterval = 0.40; // Sequential departure one after the other
    const riseDuration = 0.48;

    this.avatars.forEach((av, idx) => {
      const stepTime = idx * staggerInterval;

      // 1. Individual portal swirls open directly above this avatar
      tl.call(() => {
        playAvatarWhoosh(1.15 - idx * 0.08);
        if (av.topPortalMesh) {
          gsap.fromTo(
            av.topPortalMesh.scale,
            { x: 0.001, y: 0.001, z: 0.001 },
            { x: 1.0, y: 1.0, z: 1.0, duration: 0.28, ease: 'back.out(1.4)' }
          );
        }
      }, null, stepTime);

      // 2. Avatar rises up and gets sucked into its top portal directly above head
      tl.to(av.avatarGroup.position, {
        y: 2.55,
      ...
      tl.call(() => {
        av.avatarGroup.visible = false;
        if (av.topPortalMesh) {
          gsap.to(av.topPortalMesh.scale, { x: 0.001, y: 0.001, z: 0.001, duration: 0.28, ease: 'power2.in' });
        }
      }, null, stepTime + 0.08 + riseDuration);
    });
  }
  ...
  update(time, dt) {
    ...
    for (let i = 0; i < this.avatars.length; i++) {
      ...
      */
      ...
      // Keep top portal in the point of view of the user (facing camera)
      if (av.topPortalMesh && av.topPortalMesh.scale.x > 0.005) {
        av.topPortalMesh.quaternion.copy(this.camera.quaternion);
        if (av.topPortalMesh.children[1]) {
          av.topPortalMesh.children[1].rotation.z += 0.035;
        }
        if (av.portalWaveMesh && av.portalWaveMesh.userData) {
          const geo = av.portalWaveMesh.geometry;
          const pos = geo.attributes.position.array;
          const orig = av.portalWaveMesh.userData.orig;
          const phases = av.portalWaveMesh.userData.phases;
          const count = orig.length / 2;
          for (let pIdx = 0; pIdx < count; pIdx++) {
            const ox = orig[pIdx * 2];
            const oy = orig[pIdx * 2 + 1];
            const phase = phases[pIdx];
            pos[pIdx * 3 + 2] = 0.01 + Math.sin(ox * 4.2 + t * 2.8 + phase) * Math.cos(oy * 4.2 + t * 2.2) * 0.10;
          }
