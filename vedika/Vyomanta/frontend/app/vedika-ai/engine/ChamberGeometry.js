...
export class Chamber {
  ...
  _createCircularPedestal() {
    ...
    // 4. Subtle, Very Lite Floating Smoky Fog Layer at Disk Base (Continuous ethereal waft)
    this.fogPlanes = [];
    const fogGeo = new THREE.PlaneGeometry(5.2, 3.8);
    ...
    for (let f = 0; f < 3; f++) {
      const fogMat = new THREE.MeshBasicMaterial({
        ...
        color: this.primaryColor,
        transparent: true,
        opacity: 0.08, // Very lite subtle fog as requested
        blending: THREE.AdditiveBlending,
        depthWrite: false,
        side: THREE.DoubleSide,
      });
      const fogMesh = new THREE.Mesh(fogGeo, fogMat);
      fogMesh.position.set((f - 1) * 0.28, -1.24 + f * 0.016, 0.22 + (f - 1) * 0.14);
      fogMesh.rotation.y = f * (Math.PI / 3);
      this.pedestalGroup.add(fogMesh);

      this.fogPlanes.push({
        mesh: fogMesh,
        baseY: -1.24 + f * 0.016,
        rotSpeed: (f % 2 === 0 ? 0.045 : -0.04) * (1 + f * 0.25),
  ...
  update(time) {
    ...
    if (this.fogPlanes && this.fogPlanes.length > 0) {
      for (let i = 0; i < this.fogPlanes.length; i++) {
        ...
        p.mesh.rotation.y += p.rotSpeed * 0.012;
        p.mesh.position.y = p.baseY + Math.sin(time * 0.85 + p.phase) * 0.022;
        p.mesh.material.opacity = 0.075 + Math.sin(time * 0.65 + p.phase) * 0.02;
      }
    }
  }
