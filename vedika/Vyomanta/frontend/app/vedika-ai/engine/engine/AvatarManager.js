...
function createPortalRingTexture(themeColorHex) {
  ...
  grad.addColorStop(0.50, themeColorHex + 'cc');
  grad.addColorStop(0.72, '#FDE047'); // Golden fiery sparks
  grad.addColorStop(0.88, '#FFFFFF'); // Hot white rim
  grad.addColorStop(1.0, 'rgba(0,0,0,0)');
  ctx.fillStyle = grad;
  ctx.fillRect(0, 0, 256, 256);

  // Fiery sparks and embers orbiting the ring
  for (let i = 0; i < 48; i++) {
    const angle = (i / 48) * Math.PI * 2 + (Math.random() - 0.5) * 0.15;
    const r = 94 + (Math.random() - 0.5) * 16;
    const sx = cx + Math.cos(angle) * r;
    const sy = cy + Math.sin(angle) * r;
    ctx.fillStyle = Math.random() > 0.4 ? '#FFFFFF' : '#F59E0B';
    ctx.beginPath();
    ctx.arc(sx, sy, 1.8 + Math.random() * 2.5, 0, Math.PI * 2);
    ctx.fill();
  }
...
export class AvatarManager {
  constructor(scene, camera, chambers) {
    ...
    // Load 100% color-matched textures preserving pure white eyes:
    // Avatar 0: Purple (#bf55f7)
    // Avatar 1: Red (#f22a26)
    // Avatar 2: Gold (#b8b054)
    // Avatar 3: Electric Blue (#1f72ff)
    const textureLoader = new THREE.TextureLoader();
    this.textures = [
      textureLoader.load('/avatar_purple.webp'),
      textureLoader.load('/avatar_red.webp'),
    ...
    // Avatar 3: Electric Blue (#1f72ff)
    this.crownTextures = [
      textureLoader.load('/crown_purple.png'),
      textureLoader.load('/crown_red.png'),
  ...
  _instantiateAvatars() {
    this.chambers.forEach((chamber, idx) => {
      ...
      */
      ...
      // 6. Authentic Top Doctor Strange Portal Ring & Void Aperture directly above avatar
      const portalGroup = new THREE.Group();
      const portalGeo = new THREE.PlaneGeometry(3.0, 3.0);
      ...
      // Pitch black void inside aperture
      const voidGeo = new THREE.CircleGeometry(0.92, 32);
      ...
      portalGroup.position.set(0, 5.0, 0);
      portalGroup.scale.set(0.001, 0.001, 0.001); // starts closed
      avatarGroup.add(portalGroup);
  ...
  playPortalArrival(onAllLanded) {
    ...
    const staggerInterval = 0.42;

    this.avatars.forEach((av, idx) => {
      const delay = 0.25 + idx * staggerInterval;
      const tl = gsap.timeline({ delay });

      // 1. Portal swirls open with fiery sparks above avatar
      tl.call(() => {
        ...
        if (av.topPortalMesh) {
          gsap.fromTo(
            ...
            { x: 1.0, y: 1.0, z: 1.0, duration: 0.25, ease: 'back.out(2)' }
          );
        }
      });
    ...
    const totalLandingTime = 0.25 + (numAvatars - 1) * staggerInterval + dropDuration + 0.75;
  ...
  playPortalExit(onComplete) {
    ...
    this.avatars.forEach((av, idx) => {
      const stepTime = idx * 0.14;

      // Portal opens on top of this avatar
      tl.call(() => {
        playAvatarWhoosh(1.1 - idx * 0.08);
        if (av.topPortalMesh) {
          gsap.fromTo(
            av.topPortalMesh.scale,
            { x: 0.001, y: 0.001, z: 0.001 },
            { x: 1.0, y: 1.0, z: 1.0, duration: 0.22, ease: 'back.out(2)' }
          );
        }
      }, null, stepTime);

      // Avatar rises up and gets sucked into its top portal
      tl.to(av.avatarGroup.position, {
        y: 5.2,
        duration: 0.45,
        ease: 'power2.in',
      ...
      // Portal swirls shut into absolute void
      tl.call(() => {
        if (av.topPortalMesh) {
          gsap.to(av.topPortalMesh.scale, { x: 0.001, y: 0.001, z: 0.001, duration: 0.25, ease: 'power2.in' });
        }
      }, null, stepTime + 0.52);
    });
  }
